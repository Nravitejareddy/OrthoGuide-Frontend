...
import com.yourname.orthoguide.adapters.ClinicianPatientAdapter
        ...
        RetrofitClient.service.getClinicianPatients(clinicianId).enqueue(object : Callback<List<ClinicianPatientItem>> {
            override fun onResponse(call: Call<List<ClinicianPatientItem>>, response: Response<List<ClinicianPatientItem>>) {
                if (response.isSuccessful) {
                    ...
                    com.yourname.orthoguide.util.DialogUtils.showErrorSnackbar(findViewById(android.R.id.content), "Failed to fetch patients")
                }
            }

            override fun onFailure(call: Call<List<ClinicianPatientItem>>, t: Throwable) {
                com.yourname.orthoguide.util.DialogUtils.showErrorSnackbar(findViewById(android.R.id.content), "Error: ${t.message}")
            }
        })
    }
