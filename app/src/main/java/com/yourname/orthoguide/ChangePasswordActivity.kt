...
class ChangePasswordActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (etCurrentPassword != null && ivToggleCurrent != null) {
            com.yourname.orthoguide.util.PasswordToggleHelper.setupPasswordToggle(etCurrentPassword, ivToggleCurrent)
        }
        ...
        btnUpdatePassword?.setOnClickListener {
            ...
            if (newPw != confirmPw) {
                com.yourname.orthoguide.util.DialogUtils.showError(findViewById(android.R.id.content), "New passwords do not match")
                return@setOnClickListener
            }
    ...
    private fun performChangePassword(oldPw: String, newPw: String) {
        ...
        if (userId.isEmpty()) {
            com.yourname.orthoguide.util.DialogUtils.showError(this, "Session error. Please login again.")
            return
        }
        ...
        RetrofitClient.service.changePassword(request).enqueue(object : Callback<GenericResponse> {
            override fun onResponse(call: Call<GenericResponse>, response: Response<GenericResponse>) {
                if (response.isSuccessful) {
                    com.yourname.orthoguide.util.DialogUtils.showSuccess(this@ChangePasswordActivity, "Password updated successfully!")
                    finish()
                } else {
                    val errorMsg = response.errorBody()?.string() ?: "Failed to update password"
                    Log.e("OrthoGuide", "Password update failed: $errorMsg")
                    com.yourname.orthoguide.util.DialogUtils.showError(this@ChangePasswordActivity, "Incorrect current password or update failed")
                }
            }

            override fun onFailure(call: Call<GenericResponse>, t: Throwable) {
                Log.e("OrthoGuide", "Network error", t)
                com.yourname.orthoguide.util.DialogUtils.showError(this@ChangePasswordActivity, "Network error: ${t.message}")
            }
        })
    }
}
