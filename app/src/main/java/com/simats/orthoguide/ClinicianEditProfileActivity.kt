            ...
            override fun onResponse(call: Call<GenericResponse>, response: Response<GenericResponse>) {
                if (response.isSuccessful) {
                    getSharedPreferences("OrthoPref", Context.MODE_PRIVATE).edit()
                        ...
                        .apply()
                    com.simats.orthoguide.util.DialogUtils.showSuccessSnackbar(findViewById(android.R.id.content), "Profile updated successfully")
                    android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
                        finish()
                    }, 1000)
                } else {
                    com.simats.orthoguide.util.DialogUtils.showErrorSnackbar(findViewById(android.R.id.content), "Update failed")
                }
            }
            override fun onFailure(call: Call<GenericResponse>, t: Throwable) {
                com.simats.orthoguide.util.DialogUtils.showErrorSnackbar(findViewById(android.R.id.content), "Network error")
            }
        })
    }
}
