...
import com.simats.orthoguide.network.ReactivationRequest
...
class ReactivationRequestActivity : AppCompatActivity() {
    ...
    private fun submitRequest(userRole: String) {
        ...
        if (patientId.isEmpty() || patientName.isEmpty() || contactInfo.isEmpty() || reason.isEmpty()) {
            com.simats.orthoguide.util.DialogUtils.showError(findViewById(android.R.id.content), "Please fill all required fields")
            return
        }

        if (contactInfo.length < 5) {
            com.simats.orthoguide.util.DialogUtils.showError(findViewById(android.R.id.content), "Please enter a valid contact email/phone")
            return
        }
        ...
        RetrofitClient.service.submitReactivationRequest(request).enqueue(object : Callback<GenericResponse> {
            override fun onResponse(call: Call<GenericResponse>, response: Response<GenericResponse>) {
                ...
                if (response.isSuccessful) {
                    com.simats.orthoguide.util.DialogUtils.showSuccess(findViewById(android.R.id.content), "Request Sent Successfully")
